...
本模板按照我的个人喜好，采用较为简洁清爽的风格，去除了高鹏鸿学长的模板中关于页眉页脚的相关内容。此外，将实现代码块的宏包更改为 \mintinline{bash}{minted} ，以实现对多种语言语法高亮的支持，但也同时带来一些使用上的不便，需要对编译器及环境做一定配置，具体内容在正文部分中有所介绍。
...
本模板使用 \mintinline{bash}{minted} 宏包来实现对多种语言语法高亮的支持。由于 \mintinline{bash}{minted} 宏包依赖于Python的 \mintinline{bash}{Pygments} 包，且需要在编译时指定 \mintinline{bash}{-shell-escape} 选项，故需要对编译器及环境做一定配置。

下面的配置以 vscode 为例，其他编辑器类似。

打开 vscode 后，按下 \mintinline{bash}{Ctrl + Shift + P}，在命令面板中输入 \mintinline{bash}{settings}，选择 \mintinline{bash}{Preferences: Open User Settings (JSON)}，在打开的 \mintinline{bash}{settings.json} 文件中找到 latex-workshop.latex.tools 选项，在所使用的编译器的参数中添加 \mintinline{bash}{-shell-escape} 和 \mintinline{bash}{-8bit} 两项，如\cref{fig:2} 所示。

其中，\mintinline{bash}{-shell-escape} 选项用于允许编译器调用 \mintinline{bash}{Pygments} 环境； \mintinline{bash}{-8bit} 选项用于解决 \mintinline{bash}{minted} 宏包在中文环境下的编码问题，以避免在缩进处渲染奇怪的符号。

此外，在使用 \mintinline{bash}{minted} 宏包前，还需在环境中安装 \mintinline{bash}{Pygments} 环境。使用 \mintinline{bash}{pip} 或使用系统包管理器可完成安装，具体的安装步骤依不同的系统环境及版本可能有所变化，请搜索互联网以获得最新、最准确的安装步骤。

下面仅展示 \mintinline{bash}{minted} 宏包的简单使用方法，更多使用方式可以查看 \href{https://mirror.math.princeton.edu/pub/CTAN/macros/latex/contrib/minted/minted.pdf}{\mintinline{bash}{minted} 宏包的官方文档}。
...
请注意，在使用 \mintinline{bash}{minted} 展示 \LaTeX 代码时，代码中的 \mintinline{latex}{\end{minted}} 会使编译器无法分辨代码的结束位置，导致编译失败。为解决此问题，可将待展示的\LaTeX 代码写入一个文件中，以插入外部代码文件展示。
...
此外，本模板还使用 \mintinline{bash}{cleveref} 宏包对引用命令进行了重定义，使用 \mintinline{latex}{\cref} 命令可实现对标签的引用，使用时无需再手动添加标签类型前缀。本模板已默认将“listing”“figure”“table”三个前缀重定义为“代码”“图”“表”，相关设置可以在 \mintinline{bash}{report-style.sty} 文件中查看，可自行修改或增加其他类型前缀的重定义。
...
使用 \mintinline{bash}{fontspec} 宏包可以自定义字体。
...
使用 \mintinline{bash}{fontspec} 宏包设置的字体应已安装在系统环境中，字体名称为字体家族名称，而非字体文件名。

\subsection{颜色}

使用 \mintinline{bash}{xcolor} 宏包可以自定义颜色。

使用 \mintinline{latex}{\definecolor{colorname}{colortype}{colorvalue}} 命令定义颜色，其中 \mintinline{latex}{colorname} 为颜色名称，\mintinline{latex}{colortype} 为颜色类型，\mintinline{latex}{colorvalue} 为颜色值。常用的颜色类型有：\mintinline{bash}{rgb}（RGB 颜色）、\mintinline{bash}{cmyk}（CMYK 颜色）、\mintinline{latex}{bahs}（HTML 颜色）。
...
使用 \mintinline{bash}{geometry} 宏包可以自定义页边距。
...
本模板使用 \mintinline{bash}{minted} 宏包实现代码高亮。

\mintinline{bash}{minted} 宏包依赖于Python的 \mintinline{bash}{Pygments} 包实现代码高亮，而 \mintinline{bash}{Pygments} 包支持数百种编程语言、模板语言与标记语言，因而大部分情况下无需担心语言不支持的问题。
...
更多选项可以查看 \href{https://mirror.math.princeton.edu/pub/CTAN/macros/latex/contrib/minted/minted.pdf}{\mintinline{bash}{minted} 宏包的官方文档}

下面以\cref{lst:24} 中的代码为例，展示 \mintinline{bash}{minted} 宏包选项的设置方法，本模板也设置为下面的样式。

\begin{center}
    \captionof{listing}{\mintinline{bash}{minted} 宏包选项示例代码}
    \label{lst:24}
    \begin{minted}{latex}
\setminted{
    linenos,
